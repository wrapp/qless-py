...
class Jobs(object):
    ...
    def running(self, offset=0, count=25):
        return self.client._qless([], ['jobs', repr(time.time()), 'running', self.name, offset, count])

    def stalled(self, offset=0, count=25):
        return self.client._qless([], ['jobs', repr(time.time()), 'stalled', self.name, offset, count])

    def scheduled(self, offset=0, count=25):
        return self.client._qless([], ['jobs', repr(time.time()), 'scheduled', self.name, offset, count])

    def depends(self, offset=0, count=25):
        return self.client._qless([], ['jobs', repr(time.time()), 'depends', self.name, offset, count])

    def recurring(self, offset=0, count=25):
        return self.client._qless([], ['jobs', repr(time.time()), 'recurring', self.name, offset, count])
...
class Queue(object):
    ...
    def __getattr__(self, key):
        if key == 'jobs':
            ...
            return self.jobs
        if key == 'counts':
            return json.loads(self.client._qless([], ['queues', time.time(), self.name]))
        if key == 'heartbeat':
            a = client.config.all
    ...
    def put(self, klass, data, priority=None, tags=None, delay=None,
        ...
        return self.client._qless([], ['put', repr(time.time()), self.name,
    ...
    def pop(self, count=None):
        ...
        off.'''
        results = [Job(self.client, **j) for j in json.loads(self.client._qless([], ['pop', repr(time.time()), self.name, self.worker_name, count or 1]))]
        if count == None:
    ...
    def peek(self, count=None):
        ...
        results = [Job(self.client, **r) for r in json.loads(self.client._qless([], ['peek', repr(time.time()), self.name, count or 1]))]
        if count == None:
            return (len(results) and results[0]) or None
        return results
    ...
    def stats(self, date=None):
        ...
        return json.loads(self.client._qless([], ['stats', repr(time.time()), self.name, date or repr(time.time())]))
